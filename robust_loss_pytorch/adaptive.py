...
class AdaptiveLossFunction(nn.Module):
  ...
  def adap_alpha_scale(self, al_coef, sca_coef):
    return util.affine_sigmoid(self.latent_alpha*al_coef, lo=self.alpha_lo, hi=self.alpha_hi), \
      util.affine_softplus(self.latent_scale+sca_coef, lo=self.scale_lo, ref=self.scale_init)
  ...
  def lossfun_adap(self, x, al_coef, sca_coef, **kwargs):
    """Computes the loss on a matrix.
    ...
    assert x.shape[1] == self.num_dims
    assert x.dtype == self.float_dtype
    alpha, scale = self.adap_alpha_scale(al_coef, sca_coef)
    return self.distribution.nllfun(x, alpha, scale, **kwargs)
