...
class MyAdam:
    ...
    def cal_mean_grads(self, phi_grads):
        len_grads = len(phi_grads)
        phi_grad = list(phi_grads[0])
        for i in range(1, len_grads):
            for idx, grad in enumerate(phi_grads[i]):
                phi_grad[idx] = phi_grad[idx].add(grad)
        for idx in range(len(phi_grad)):
            phi_grad[idx] = phi_grad[idx] / len_grads
        return phi_grad
