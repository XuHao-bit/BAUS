...
config_item_ds = {
    'movielens': ['rate', 'year', 'genre', 'director'],
    'bookcrossing': ['author', 'year', 'publisher'],
    'dbook': ['author', 'publisher', 'year']
}
config_item_size = {
    'movielens': [6, 81, 25, 2186],
    'bookcrossing':[84725, 97, 13896],
    'dbook':[9913, 1636, 65]
}
...
def perturb_item(sup_x, dataset, prob=0.3):
    attr_size = config_item_size[dataset]
    attr_list = config_item_ds[dataset]

    # prob = 0.3
    row_mask = (torch.rand(sup_x.size(0)) < prob).nonzero().squeeze(1) # select the sample
    col_mask = torch.randint(low=0, high=len(attr_list), size=row_mask.size())
    for row_idx, col_idx in zip(row_mask, col_mask):
        v = random.randint(0, attr_size[col_idx] - 1) # randint为闭区间
        if dataset == 'movielens' and col_idx == 2: # genre 
            sup_x[row_idx, 2:27] = 0
            sup_x[row_idx, v + 2] = 1
        elif dataset == 'movielens' and col_idx == 3: # director
            sup_x[row_idx, 27:] = 0
            sup_x[row_idx, v + 27] = 1
        else:
            sup_x[row_idx, col_idx] = v
    return sup_x


def perturb_item2(sup_y, dataset, prob=0.3):
    row_mask = (torch.rand(sup_y.size(0)) < prob).nonzero().squeeze(1) # select the sample
    
    for row_idx in row_mask:
        if sup_y[row_idx] > dataset_info[dataset]['num_class'] / 2:
            sup_y[row_idx] = 0
        else:
            sup_y[row_idx] = dataset_info[dataset]['num_class'] - 1
