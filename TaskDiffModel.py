...
# from modules.ModelModulator import Modulated_Decoder
from modules.TaskEncoder import *
...
class TaskDiffModel(nn.Module):
    def __init__(self, dataset, emb_info, dataset_info, config_settings) -> None:
        ...
        if self.use_sslencoder and not self.use_tmem:
            self.task_encoder = TaskEncoderSSL(self.task_dim, self.num_class, self.emb_size, \
                                           self.feat_num, self.max_shot, self.clusters_k, self.temperature, self.device)
        elif self.use_sslencoder and self.use_tmem:
            self.task_encoder = TaskEncoderSSLMem(self.task_dim, self.num_class, self.emb_size, \
                                           self.feat_num, self.max_shot, self.clusters_k, self.temperature, self.device, config=config_settings)
        else:
            self.task_encoder = TaskMeanMemEncoderSSL(self.task_dim, self.num_class, self.emb_size,  \
                                            self.feat_num, self.max_shot, self.clusters_k, self.temperature, self.device, config=config_settings)
    ...
    # def get_base_model_params_dict(self):
    #     # return self.decoder.get_param_dict() 用哪个？？
    #     return self.base_model.decoder.get_param_dict()
    ...
    def get_task_embedding(self, sup_x1, sup_x2, sup_y):
        ...
        # print(sup_y.int())
        task_emb = self.task_encoder(x_emb, sup_y.long())
        return task_emb

    def get_task_emb_nossl(self, sup_x1, sup_x2, sup_y):
        x_emb = self.base_model.get_embedding(sup_x1, sup_x2)
        return self.task_encoder.forward_nossl(x_emb, sup_y.long())
    
    def get_contrastive_taskemb(self, sup_x1, sup_x2, sup_y):
        x_emb = self.base_model.get_embedding(sup_x1, sup_x2)
        return self.task_encoder.forward_on_tar(x_emb, sup_y.long())
    
    def get_sslview_taskemb(self, sup_x1, sup_x2, sup_y):
        x_emb = self.base_model.get_embedding(sup_x1, sup_x2)
        return self.task_encoder.gen_ssl_on_tar(x_emb, sup_y.long())
